...
class Voter extends Component {
  ...
  render() {
    const size = this.props.type === 'articles' ? 'fa-2x' : '';
    return (
      <div>
        {this.state.voteMod >= 1 ? (
          <i className={`fas fa-arrow-alt-circle-up ${size} votingDisabled`} />
        ) : (
        ...
        {this.state.voteMod <= -1 ? (
          <i
            className={`fas fa-arrow-alt-circle-down ${size} votingDisabled`}
          />
        ) : (
  ...
  voteHandler = upOrDown => {
    ...
    api
      .changeVotes(`/articles/${this.props.componentInfo._id}`, upOrDown)
